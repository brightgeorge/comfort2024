...
def user_dashboard_calculations_ob_ch9(request):
    if 'username' in request.session:
        us = request.session['username']
        a = admin_dashboard_calculations_br9.grand_total_collection()
        from datetime import datetime
    ...
    return render(request, 'branches/branch9/user_dashboard_calculations.html', context)
    return render(request, 'index.html')
    ...
    if 'username' in request.session:
        if request.method == 'POST':
            ...
            if chk_mob == True:
                ...
                messages.info(request, 'BRANCH 9 guest already exists')
                ...
                print('l', l)
                context = {
                    'brname': 'BRANCH9 Room Creation Form',
                    'br': pg1_new_beds.objects.all().filter(roon_no=1).order_by('roon_no'),
                    'rn1': l[0]
                }
...
def multiple_br1_admit_guest9(request, id):
    if 'username' in request.session:
        if request.method == 'POST':
            selfmob = request.POST.get('selfmobno')
            chk_mob = pg1_new_guest.objects.all().filter(self_mob=selfmob).exists()
...
def new_year_jan_print9(request):
    if 'username' in request.session:
        l = []
        data = pg1_new_beds.objects.all()
        for i in data:
        ...
        return render(request, 'branches/branch9/print_outs/new_year_jan_print.html', context)
    return render(request, 'index.html')
