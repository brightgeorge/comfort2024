...
def multiple_branch1_room_create_regi12(request):
    if 'username' in request.session:
        if request.method == 'POST':
            chk_room_no = request.POST.get('roonno')
            chk_room = room_pg1.objects.all().filter(roon_no=chk_room_no).exists()
            if chk_room == True:
                ...
                messages.info(request, 'BRANCH12 roon no already exists')
                return render(request, 'branches/branch12/rooms/view_all_rooms.html', context)
            else:
                rnum = [
                ...
                messages.info(request, 'BRANCH12 room created sucessfully')
                return render(request, 'branches/branch12/rooms/view_all_rooms.html', context)
    return render(request, 'index.html')
...
def multiple_single_pg1_bed_create_regi12(request):
    if 'username' in request.session:
        if request.method == 'POST':
            ...
            for i in range(len(a)):
                ...
                if chk_bd_code == True:
                    ...
                    messages.info(request, 'BRANCH12 bed no already exists')
                    return render(request, 'branches/branch12/beds/view_all_beds.html', context)
                else:
                    room_no = a[i]
                    room_name = b[i]
